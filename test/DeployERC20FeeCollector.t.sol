...
contract DeployERC20FeeCollectorTestBase is DeployERC20FeeCollectorBaseImpl {
  ...
  function run() public override returns (ERC20FeeCollector) {
    return _deploy(payoutReceiver, payoutToken, payoutAmount);
  }
}
...
contract DeployERC20FeeCollectorTest is Test {
  function testFuzz_CorrectlyDeploysErc20FeeCollector(
    ...
    DeployERC20FeeCollectorTestBase deployer =
      new DeployERC20FeeCollectorTestBase(payoutReceiver, payoutToken, payoutAmount);
    ...
    assertEq(deployed.PAYOUT_AMOUNT(), payoutAmount);
  }
}
