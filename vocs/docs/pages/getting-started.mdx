...
In the current version of the Phoenix Mechanism, searchers can *permissionlessly* capture value from the system
...
:::note
The guide below assumes participants are interacting with the `Firepit` contract from a wallet

For a contract-interaction guide, see [Flash Borrow](/guides/flash-borrow)
:::
...
Participants can view the `threshold` by calling [`Firepit.threshold()`](/technical-reference/interface.IResourceManager#threshold)
...
Because the `Firepit` contract transfers UNI to `address(0xdead)` integrating addresses must first *approve* the `Firepit` to spend their UNI
...
Once the value of the assets exceeds the value of the UNI tokens, integrators should call [`Firepit.release()`](/technical-reference/interface.IReleaser#release)
