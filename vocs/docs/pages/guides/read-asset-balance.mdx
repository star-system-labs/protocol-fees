...
As noted in [Best Practices](/guides/best-practices), the `TokenJar's` available asset balance is observed into two places:

* contract address of the `TokenJar` itself
* the `UniswapV3Pool` contract addresses

For example, the *releasable* USDC balance is `USDC.balanceOf(address(tokenJar)) + sum of UniswapV3Pool.protocolFees()` for each USDC-paired pool
...
import {ITokenJar} from "@uniswap/protocol-fees/src/interfaces/ITokenJar.sol";

contract Example {
    ITokenJar tokenJar = ITokenJar(0x0);
    ...
    function releaseableBalance(address token, address[] calldata pools)
        ...
        returns (uint256)
    {
        return IERC20(token).balanceOf(address(tokenJar)) // [!code hl]
...
Uniswap v2 protocol fees are automatically "pushed" to the `TokenJar` so no additional calls are required to make the assets releasable.
...
uint256 amount0 = (lpToken.balanceOf(address(tokenJar)) * IERC20(lpToken.token0()).balanceOf(address(lpToken))) / lpToken.totalSupply();
uint256 amount1 = (lpToken.balanceOf(address(tokenJar)) * IERC20(lpToken.token1()).balanceOf(address(lpToken))) / lpToken.totalSupply();
```
