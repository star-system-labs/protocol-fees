...
Below, please find a list of best practices when interacting with the ProtocolFees system.
...
contract SafeRelease {
    ...
    function vulnerable(Currency[] memory assets) external { // [!code --]
    // ✅ THIS IS SAFE FROM FRONT RUNNING ✅ // [!code ++]
    function safeRelease(uint256 _nonce, Currency[] memory assets) external { // [!code ++]
        uint256 nonce = firepit.nonce(); // [!code --]
        firepit.release(_nonce, assets, address(this));
    }
}
```
In the safe pattern, the caller is responsible for reading the `Firepit.nonce()` offchain, assocating it with releasable assets, and passing it as a parameter.

## Collect Uniswap v3 Fees

For further reading, please see [Read Asset Balance](/guides/read-asset-balance)

In the ProtocolFees system, Uniswap v3 fees *must be collected* to the `TokenJar` before they are eligible for release.
...
[IV3FeeAdapter.collect()](/technical-reference/IV3FeeAdapter#collect)
...
The ProtocolFees system allows for an *updatable* [`Firepit.threshold()`](/technical-reference/IResourceManager#threshold)
...
Because the ProtocolFees system can release native tokens (Ether), recipients of the tokens should be `payable`
...
Uniswap v2 Protocol Fees are automatically pushed to the `TokenJar` contract, but are represented as ERC-20 LP tokens. These ERC-20 LP tokens represent a combination of `token0` and `token1`
